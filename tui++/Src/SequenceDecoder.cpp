...
SequenceDecoder::SequenceDecoder(session_ptr session) :
    m_weak_session(session), m_sequence_state(SEQ_NORMAL), m_sequence(0), m_parameter(0), m_is_sequence(false), m_is_parameter(false), m_is_invalid_sequence(false), m_is_sequence_completed(false), m_sequence_level(0) {
...
void SequenceDecoder::validateSequence() {
  ...
  try {
    ...
    int sequenceTerminator = m_sequence_builder[m_sequence_builder.size() - 1];
    ...
    std::cout << "Exception removing ESC and Terminator Sequences: " << e.what() << std::endl;
  }
  ...
  if (result != std::string::npos) {
    ...
    while (std::getline(inStream, token, ';')) {
      ...
      {
        // Skip this character.
        std::cout << "Exception String to Int Parameter Failure." << std::endl;
        continue;
      }
    ...
    if (m_sequence_builder[0] == '?') {
      try {
        ...
        std::cout << "Exception removing ? sequence starter: " << e.what() << std::endl;
      }
    }
  ...
  while (result != std::string::npos) {
    ...
    if (result != std::string::npos) {
      try {
        ...
        std::cout << "Exception replacing double ESC characters: " << e.what() << std::endl;
    ...
    if (m_sequence == '\x1b' && !m_is_sequence_completed) {
      ...
      if (m_is_sequence_completed) {
        ...
        try {
          ...
          std::cout << "Exception substring on escapeSequence: " << e.what() << std::endl;
      ...
      if (m_is_invalid_sequence) {
        ...
        try {
          ...
          std::cout << "Exception substring on ESC Position: " << e.what() << std::endl;
        }
        // Reset the State
        m_sequence_state = SEQ_ERROR;
      }
