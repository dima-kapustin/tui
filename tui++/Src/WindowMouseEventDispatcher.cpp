...
void WindowMouseEventDispatcher::retarget_mouse_event(const std::shared_ptr<Component> &target, E &e) {
...
std::shared_ptr<Component> WindowMouseEventDispatcher::retarget_mouse_enter_exit(const std::shared_ptr<Component> &target_over, E &e, const std::shared_ptr<Component> &last_entered, bool mouse_over_window) {
  ...
  if (last_entered != target_enter) {
    ...
    if (std::is_same_v<E, MouseOverEvent> and e.type == MouseOverEvent::MOUSE_EXITED) {
    ...
    if (std::is_same_v<E, MouseOverEvent> and e.type == MouseOverEvent::MOUSE_ENTERED) {
